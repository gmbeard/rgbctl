...
auto app() -> void
{
    ...
    for (auto& item : scan) {
        ...
        char const* sysname = udev_device_get_sysname(device.get());
        ...
        DeviceDescriptor descriptor {
            .device_name = sysname,
            .device_node = devname,
            .product_name = uevent_map["HID_NAME"],
            .bus_id = 0,
            .vendor_id = 0,
            .product_id = 0,
        };
        ...
        parse_device_id(
            uevent_map["HID_ID"], descriptor.vendor_id, descriptor.product_id);
        devices.push_back(std::move(descriptor));
    }

    for (auto const& dev : devices) {
        std::cout << "NAME: " << dev.device_name << '\n'
